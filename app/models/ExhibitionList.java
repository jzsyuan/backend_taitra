...
@Entity(name = "\"EXHIBITION_LIST\"")
@Table(name = "\"EXHIBITION_LIST\"")
public class ExhibitionList extends BaseModel {
    @Id
    ...
    public static ExhibitionList findExhibitionListById(String exhibitionId) {
       return getEbeanServer().createQuery(ExhibitionList.class).where().eq("\"EXHIBITION_ID\"", exhibitionId).findOne();
    }

    public static List<ExhibitionList> findExhibitionAll() {
        ExpressionList<ExhibitionList> expressionList = getEbeanServer().createQuery(ExhibitionList.class).where();
        return expressionList.findList();
    }

    public static List<ExhibitionList> findExhibitionList(String keyword, int year, String nation, Date now, List<String> exhibitionIds, List<String> notExhibitionIds, boolean isDescending, int pageIndex, int pageSize) {
        ExpressionList<ExhibitionList> expressionList = getEbeanServer().createQuery(ExhibitionList.class).where();
    ...
    public static List<ExhibitionList> findByExhibitionIds(List<String> exhibitionIds, int pageIndex, int pageSize) {
        ExpressionList<ExhibitionList> expressionList = getEbeanServer().createQuery(ExhibitionList.class).where();
    ...
    public static List<ExhibitionList> findByGroup(String groupId) {
        return getEbeanServer().createQuery(ExhibitionList.class).where()
                .eq("\"EXHIBITION_GROUP\"", groupId)
                .orderBy("\"EXHIBITION_START_DATE\" DESC")
                .findList();
    }

    public static int findExhibitionCount(String keyword, int year, String nation, Date now, List<String> exhibitionIds, List<String> notExhibitionIds, boolean isDescending) {
        ExpressionList<ExhibitionList> expressionList = getEbeanServer().createQuery(ExhibitionList.class).where();
        if (keyword != null && !keyword.isEmpty()) {
            //expressionList.like("\"EXHIBITION_TITLE\"", "%" + keyword + "%");
            expressionList.or(Expr.like("\"EXHIBITION_TITLE\"", "%" + keyword + "%"), Expr.raw("\"EXHIBITION_TAG\" ??| array['" + keyword + "']"));
